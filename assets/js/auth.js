/**
 * ==========================================================================
 * سمت SAMT - نظام المصادقة وإدارة الصلاحيات المركزي (RBAC & Auth Engine)
 * ==========================================================================
 * 1. تسجيل الدخول وإنشاء الحسابات الموحد للعملاء والمشرفين.
 * 2. التعرف على رتبة المشرف (Admin) وتفعيل أدوات التعديل والحذف الحية.
 * 3. نافذة تسجيل الدخول العصرية (Login/Register Modal).
 * 4. إدارة الجلسة وتحديث شريط التنقل (Navbar) في كافة الصفحات.
 */
...
(function () {
  ...
  const SamtAuth = {
    ...
    // إنشاء حساب جديد
    register: function (name, email, password, adminPasscode) {
      const users = initUsers();
      const cleanEmail = (email || '').trim().toLowerCase();
      ...
      return {
        success: true,
        user: sessionData,
        message: isGrantingAdmin ? 'تم إنشاء الحساب برتبة مشرف (Admin) بنجاح!' : 'تم إنشاء الحساب بنجاح!'
      };
    },
  ...
  // حقن نافذة تسجيل الدخول في الصفحة (Universal Auth Modal)
  function injectAuthModal() {
    ...
    <div id="samtAuthModal" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-md hidden items-center justify-center p-4">
      <div class="glass-card rounded-2xl w-full max-w-md overflow-hidden shadow-2xl border border-white/10 text-right">
        ...
        <!-- Auth Tabs -->
        <div class="flex border-b border-white/10 bg-black/20 text-xs font-bold">
        ...
        <div class="p-6 space-y-4">
          ...
          <form id="samtLoginForm" onsubmit="window.SamtAuth.handleLoginSubmit(event)" class="space-y-4">
            ...
            <div>
              <div class="flex items-center justify-between mb-1.5">
                ...
                <button type="button" onclick="window.SamtAuth.fillDemoAdmin()" class="text-[10px] text-samt-cyan/80 hover:text-samt-cyan">تجربة حساب الأدمن</button>
              </div>
              <div class="relative">
          ...
          <!-- REGISTER FORM -->
          <form id="samtRegisterForm" onsubmit="window.SamtAuth.handleRegisterSubmit(event)" class="space-y-4 hidden">
            <div>
              <label class="block text-xs font-bold mb-1.5 font-cairo text-slate-300">الاسم الكامل:</label>
              <div class="relative">
                <input type="text" id="authRegName" required placeholder="مثال: أحمد سامي" class="w-full glass-card border border-white/15 focus:border-samt-cyan text-xs rounded-xl py-3 pr-10 pl-3 outline-none" />
            ...
            <div>
              <label class="block text-xs font-bold mb-1.5 font-cairo text-slate-300">البريد الإلكتروني:</label>
              <div class="relative">
                <input type="email" id="authRegEmail" required placeholder="name@example.com" class="w-full glass-card border border-white/15 focus:border-samt-cyan text-xs rounded-xl py-3 pr-10 pl-3 outline-none text-left" dir="ltr" />
                <i class="fa-solid fa-envelope absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
              </div>
            </div>

            <div>
              <label class="block text-xs font-bold mb-1.5 font-cairo text-slate-300">كلمة المرور:</label>
              <div class="relative">
                <input type="password" id="authRegPassword" required placeholder="6 أحرف على الأقل" class="w-full glass-card border border-white/15 focus:border-samt-cyan text-xs rounded-xl py-3 pr-10 pl-3 outline-none text-left" dir="ltr" />
                <i class="fa-solid fa-lock absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
              </div>
            </div>
            ...
            <div id="authRegError" class="hidden p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs"></div>
  ...
  // تحديث شريط التنقل (Navbar) تلقائياً في جميع الصفحات
  function updateNavbars() {
    const user = SamtAuth.getCurrentUser();
    const isAdmin = SamtAuth.isAdmin();

    // البحث عن أزرار تسجيل الدخول أو حساب المستخدم المخصص
    const navSlots = document.querySelectorAll('.samt-auth-slot');
    
    // إخفاء أي رابط قديم للأدمن إذا لم يكن أدمن مسجل دخول
    ...
    navSlots.forEach(slot => {
      if (!user) {
        // زائر غير مسجل: زر تسجيل الدخول القياسي الأنيق
        slot.innerHTML = `
        ...
        // مستخدم أو أدمن مسجل دخول
        const isAdm = user.role === 'admin';
        slot.innerHTML = `
          <div class="relative group">
            <button class="px-3.5 py-1.5 rounded-full glass-card border ${isAdm ? 'border-samt-gold/40 text-samt-gold' : 'border-samt-cyan/30 text-white'} text-xs font-bold flex items-center gap-2 shadow-md">
            ...
            <!-- Dropdown Menu -->
            <div class="absolute left-0 mt-2 w-48 glass-card rounded-2xl shadow-2xl border border-white/10 py-2 hidden group-hover:block z-50 text-right backdrop-blur-xl">
              <div class="px-4 py-2 border-b border-white/10">
                ...
                <div class="mt-1 inline-block px-2 py-0.5 rounded-full text-[9px] font-extrabold ${isAdm ? 'bg-samt-gold/20 text-samt-gold border border-samt-gold/30' : 'bg-samt-cyan/20 text-samt-cyan border border-samt-cyan/30'}">
                  ${isAdm ? 'مشرف المنظومة (Admin)' : 'حساب مستخدم'}
                </div>
              </div>

              ${isAdm ? `
                <a href="./admin.html" class="block px-4 py-2 text-xs text-samt-gold hover:bg-white/5 font-bold">
                  <i class="fa-solid fa-chart-line ml-2"></i> لوحة التحكم الشاملة
                </a>
                <button onclick="window.SamtAuth.openNewCourseInline()" class="w-full text-right px-4 py-2 text-xs text-samt-cyan hover:bg-white/5 font-bold">
                  <i class="fa-solid fa-plus-circle ml-2"></i> إضافة كورس جديد
                </button>
                <button onclick="window.SamtAuth.exportDataFile()" class="w-full text-right px-4 py-2 text-xs text-slate-300 hover:bg-white/5 font-semibold">
                  <i class="fa-solid fa-download ml-2"></i> تصدير البيانات (Export)
                </button>
              ` : ''}

              <a href="./courses.html" class="block px-4 py-2 text-xs text-slate-300 hover:bg-white/5 font-semibold">
                <i class="fa-solid fa-graduation-cap ml-2"></i> مكتبة الكورسات
              </a>

              <div class="border-t border-white/10 my-1"></div>
              <button onclick="window.SamtAuth.logout()" class="w-full text-right px-4 py-2 text-xs text-rose-400 hover:bg-rose-500/10 font-bold">
    ...
    // إذا كان أدمن مسجل دخول، نقوم بتفعيل شريط الأدمن العلوي وميزات التعديل
    if (isAdmin) {
      injectAdminTopBar();
    }
  }
  ...
  window.SamtAuth = {
    ...
    handleRegisterSubmit: function (e) {
      ...
      const res = SamtAuth.register(name, email, pass, code);
      if (res.success) {
        this.closeAuthModal();
